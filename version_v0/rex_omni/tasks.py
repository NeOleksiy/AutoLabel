...
class TaskConfig:
    ...
    requires_keypoint_type: bool = False
            ...
            # Create a comprehensive object detection system for urban scene analysis that identifies and localizes seven critical infrastructure classes: ['car', 'door', 'handrail', 'sidewalk', 'street_light', 'staircase', 'window'].
            # Key detection requirements:
            # - **Cars**: Detect all visible vehicles including sedans, SUVs, trucks, focusing on complete body轮廓
            # - **Doors**: Identify entrance doors of buildings with clear rectangular shapes and handles
            # - **Handrails**: Detect safety railings along stairs, ramps, and elevated walkways
            # - **Sidewalks**: Segment pedestrian walkways adjacent to roads with clear boundaries
            # - **Staircases**: Locate stair structures with multiple steps and railings
            # - **Street Lights**: Identify vertical lighting poles with luminaire heads
            # - **Windows**: Detect transparent openings in building facades with glass surfaces
            
            # Prioritize detection accuracy for safety-critical elements (handrails, staircases) and ensure robust performance across varying lighting conditions, scales, and urban environments.
            # Create a binary occupancy detection system that distinguishes between 'occupied' and 'empty' states across multiple contexts with high accuracy.
            # **OCCUPIED State Detection:**
            # - Human presence: Full or partial body visibility, movement patterns
            # - Vehicle occupancy: Visible passengers/drivers in cars, buses, vehicles
            # - Seat utilization: Person sitting/using chairs, benches, seats
            # - Workspace activity: People at desks, stations, or work areas
            # - Object interaction: Human engagement with surrounding environment
            
            # **EMPTY State Detection:**
            # - Absence of human figures in defined spaces
            # - Unoccupied vehicles with empty seats
            # - Vacant chairs, benches, and sitting areas
            # - Unused workspaces and stations
            # - Static environments without human activity
...
TASK_CONFIGS: Dict[TaskType, TaskConfig] = {
    TaskType.DETECTION: TaskConfig(
        ...
        description="""
            Detect a cow with a top only if it is present with its body in the frame. Skip cows that are not fully present in the frame.
        """,
        output_format="boxes",
    ...
    TaskType.KEYPOINT: TaskConfig(
        ...
        description="""
            Detect only those cows that are completely present in the frame. Don't isolate cow body parts into a different class. Make sure the entire cow is visible in the image. 
        """,
